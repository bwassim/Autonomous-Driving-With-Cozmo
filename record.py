...
# data_dir = 'data_train_01'
data_dir = 'data_val_01'
...
def run(sdk_conn):
    ...
    robot.set_head_angle(autodrive_constants.HEAD_ANGLE,
        ...
        if joystickMoved:
            if abs(joystick.throttle) < 0.1:
                ...
                if joystick.throttle > 0.0:
                    ...
                    direction = -1
                l_wheel_speed = (
                    direction * autodrive_constants.RECORD_DRIVE_SPEED) + (joystick.x * 75.0)
                r_wheel_speed = (
                    direction * autodrive_constants.RECORD_DRIVE_SPEED) - (joystick.x * 75.0)
                robot.drive_wheel_motors(
                    l_wheel_speed, r_wheel_speed, l_wheel_acc=500, r_wheel_acc=500)
    ...
    # Save images and steering inputs3
    if len(images) > 0:
        print('Saving images')
        img_arr = np.zeros(
            (len(images), imgSize[0], imgSize[1], imgSize[2]), dtype=np.float16)
